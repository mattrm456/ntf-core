...
BSLS_IDENT_RCSID(ntsa_igmpheader_t_cpp, "$Id$ $CSID$")
...
// Provide tests for 'ntsa::IgmpHeader'.
class IgmpHeaderTest
{
  public:
...
NTSCFG_TEST_FUNCTION(ntsa::IgmpHeaderTest::verifyTypeTraits)
{
    const bool isBitwiseInitializable =
        NTSCFG_TYPE_CHECK_BITWISE_INITIALIZABLE(ntsa::IgmpHeader);

    NTSCFG_TEST_TRUE(isBitwiseInitializable);

    const bool isBitwiseMovable =
        NTSCFG_TYPE_CHECK_BITWISE_MOVABLE(ntsa::IgmpHeader);

    NTSCFG_TEST_TRUE(isBitwiseMovable);

    const bool isBitwiseCopyable =
        NTSCFG_TYPE_CHECK_BITWISE_COPYABLE(ntsa::IgmpHeader);
