...
BSLS_IDENT_RCSID(ntsa_ethernettag_t_cpp, "$Id$ $CSID$")
...
// Provide tests for 'ntsa::EthernetTag'.
class EthernetTagTest
{
  public:
...
NTSCFG_TEST_FUNCTION(ntsa::EthernetTagTest::verifyTypeTraits)
{
    const bool isBitwiseInitializable =
        NTSCFG_TYPE_CHECK_BITWISE_INITIALIZABLE(ntsa::EthernetTag);

    NTSCFG_TEST_TRUE(isBitwiseInitializable);

    const bool isBitwiseMovable =
        NTSCFG_TYPE_CHECK_BITWISE_MOVABLE(ntsa::EthernetTag);

    NTSCFG_TEST_TRUE(isBitwiseMovable);

    const bool isBitwiseCopyable =
        NTSCFG_TYPE_CHECK_BITWISE_COPYABLE(ntsa::EthernetTag);
