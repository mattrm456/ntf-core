...
BSLS_IDENT_RCSID(ntsa_udpfragmentation_t_cpp, "$Id$ $CSID$")
...
NTSCFG_TEST_FUNCTION(ntsa::UdpFragmentationTest::verifyTypeTraits)
{
    const bool isBitwiseInitializable =
        NTSCFG_TYPE_CHECK_BITWISE_INITIALIZABLE(ntsa::UdpFragmentation);

    NTSCFG_TEST_TRUE(isBitwiseInitializable);

    const bool isBitwiseMovable =
        NTSCFG_TYPE_CHECK_BITWISE_MOVABLE(ntsa::UdpFragmentation);

    NTSCFG_TEST_TRUE(isBitwiseMovable);

    const bool isBitwiseCopyable =
        NTSCFG_TYPE_CHECK_BITWISE_COPYABLE(ntsa::UdpFragmentation);
