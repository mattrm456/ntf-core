...
BSLS_IDENT_RCSID(ntsa_arptype_t_cpp, "$Id$ $CSID$")
...
// Provide tests for 'ntsa::ArpType'.
class ArpTypeTest
{
  public:
...
NTSCFG_TEST_FUNCTION(ntsa::ArpTypeTest::verifyFromInt)
{
    typedef ntsa::ArpType T;

    // clang-format off
    struct TestData {
        int                    d_result;
        int                    d_input;
        ntsa::ArpType::Value  d_output;
    };
    ...
    for (bsl::size_t i = 0; i < k_TEST_DATA_COUNT; ++i) {
        ...
        ntsa::ArpType::Value value;
        int rc = ntsa::ArpType::fromInt(&value, testData.d_input);
...
NTSCFG_TEST_FUNCTION(ntsa::ArpTypeTest::verifyFromString)
{
    typedef ntsa::ArpType T;
    ...
    const TestData k_TEST_DATA[] = {
        ...
        {  0, "REQUEST",   T::e_REQUEST   },
        {  0, "RESPONSE",  T::e_RESPONSE  },
        { -1, "!",         T::e_UNDEFINED }
    };
    // clang-format on
    ...
    for (bsl::size_t i = 0; i < k_TEST_DATA_COUNT; ++i) {
        ...
        ntsa::ArpType::Value value;
        int rc = ntsa::ArpType::fromString(&value, testData.d_input);
...
NTSCFG_TEST_FUNCTION(ntsa::ArpTypeTest::verifyPrint)
{
    // clang-format off
    struct TestData {
        const char* d_result;
    ...
    for (bsl::size_t i = 0; i < k_TEST_DATA_COUNT; ++i) {
        ...
        const ntsa::ArpType::Value input =
            static_cast<ntsa::ArpType::Value>(testData.d_input);
