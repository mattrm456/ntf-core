...
NTSCFG_TEST_FUNCTION(ntsa::Ipv4PacketTest::verifySerializationUdpIpv4)
{
    ...
    NTSCFG_TEST_EQ(blobBuffer.size(), k_DATA_SIZE);

    bsl::memcpy(blobBuffer.data(), k_DATA, k_DATA_SIZE);

    ntsa::Ipv4Packet ipv4Packet;
    error = ipv4Packet.decode(blobBuffer);
    NTSCFG_TEST_OK(error);

    BALL_LOG_DEBUG << "Packet = " << ipv4Packet << BALL_LOG_END;

    NTSCFG_TEST_EQ(ipv4Packet.header().headerLength(), 20);
    NTSCFG_TEST_EQ(ipv4Packet.header().packetLength(), 41);
    NTSCFG_TEST_EQ(ipv4Packet.header().id(), 16137);
    NTSCFG_TEST_EQ(ipv4Packet.header().fragmentOffset(), 64);
    NTSCFG_TEST_EQ(ipv4Packet.header().timeToLive(), 64);
    NTSCFG_TEST_EQ(ipv4Packet.header().protocol(), 17);
    NTSCFG_TEST_EQ(ipv4Packet.header().checksum(), 64952);
    NTSCFG_TEST_EQ(ipv4Packet.header().sourceAddress(), ntsa::Ipv4Address::loopback());
    NTSCFG_TEST_EQ(ipv4Packet.header().destinationAddress(), ntsa::Ipv4Address::loopback());

    NTSCFG_TEST_TRUE(ipv4Packet.payload().isUdp());

    const ntsa::UdpPacket& udpPacket = ipv4Packet.payload().udp();

    NTSCFG_TEST_EQ(udpPacket.header().sourcePort(), 58377);
    NTSCFG_TEST_EQ(udpPacket.header().destinationPort(), 56145);
    NTSCFG_TEST_EQ(udpPacket.header().packetLength(), 21);
    NTSCFG_TEST_EQ(udpPacket.header().checksum(), 65064);
}
