...
BSLS_IDENT_RCSID(ntsa_tcpchecksum_cpp, "$Id$ $CSID$")
...
TcpChecksum::TcpChecksum(const TcpChecksum& original)
...
TcpChecksum& TcpChecksum::operator=(const TcpChecksum& other)
{
    if (this != &other) {
...
void TcpChecksum::reset()
{
    d_accumulator = 0;
}

void TcpChecksum::add(const ntsa::Ipv4Address& sourceAddress,
                      ...
                      bsl::size_t              length)
{
    ...
    d_accumulator += static_cast<bsl::uint16_t>(TcpChecksum::k_PROTOCOL_TCP);
    d_accumulator += static_cast<bsl::uint16_t>(length);
}

void TcpChecksum::add(const void* data, bsl::size_t size)
{
...
bsl::uint32_t TcpChecksum::accumulator() const
{
    return d_accumulator;
}

bsl::uint16_t TcpChecksum::value() const
{
