...
BSLS_IDENT_RCSID(ntsa_icmpredirect_cpp, "$Id$ $CSID$")
...
ntsa::Error IcmpRedirect::decode(ntsa::PacketDecoder* decoder)
{
    ntsa::Error error;

    reset();

    error = decoder->decodeRaw(&d_gatewayAddress, sizeof d_gatewayAddress);
    if (error) {
        return error;
    }
...
ntsa::Error IcmpRedirect::encode(ntsa::PacketEncoder* encoder) const
{
    ntsa::Error error;

    error = encoder->encodeRaw(&d_gatewayAddress, sizeof d_gatewayAddress);
    if (error) {
        return error;
    }
...
ntsa::Error IcmpRedirect::decode(const bdlbb::BlobBuffer& buffer,
                                 bsl::size_t              offset,
...
ntsa::Error IcmpRedirect::encode(bdlbb::BlobBuffer* buffer,
...
bsl::ostream& IcmpRedirect::print(bsl::ostream& stream,
                                  int           level,
...
void IcmpRedirect::print(bslim::Printer* printer) const
{
    printer->printAttribute("gatewayAddress", d_gatewayAddress);
    printer->printAttribute("header", d_header);
    if (d_payloadSize > 0) {
        printer->printForeign(
            bslstl::StringRef(reinterpret_cast<const char*>(d_payloadData),
                              d_payloadSize),
            &IcmpRedirect::printData,
            "payload");
    }
}
