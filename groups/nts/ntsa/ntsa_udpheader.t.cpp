...
BSLS_IDENT_RCSID(ntsa_udpheader_t_cpp, "$Id$ $CSID$")
...
// Provide tests for 'ntsa::UdpHeader'.
class UdpHeaderTest
{
  public:
...
NTSCFG_TEST_FUNCTION(ntsa::UdpHeaderTest::verifyTypeTraits)
{
    const bool isBitwiseInitializable =
        NTSCFG_TYPE_CHECK_BITWISE_INITIALIZABLE(ntsa::UdpHeader);

    NTSCFG_TEST_TRUE(isBitwiseInitializable);

    const bool isBitwiseMovable =
        NTSCFG_TYPE_CHECK_BITWISE_MOVABLE(ntsa::UdpHeader);

    NTSCFG_TEST_TRUE(isBitwiseMovable);

    const bool isBitwiseCopyable =
        NTSCFG_TYPE_CHECK_BITWISE_COPYABLE(ntsa::UdpHeader);
