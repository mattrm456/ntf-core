...
BSLS_IDENT_RCSID(ntsa_arpheader_t_cpp, "$Id$ $CSID$")
...
// Provide tests for 'ntsa::ArpHeader'.
class ArpHeaderTest
{
  public:
...
NTSCFG_TEST_FUNCTION(ntsa::ArpHeaderTest::verifyTypeTraits)
{
    const bool isBitwiseInitializable =
        NTSCFG_TYPE_CHECK_BITWISE_INITIALIZABLE(ntsa::ArpHeader);

    NTSCFG_TEST_TRUE(isBitwiseInitializable);

    const bool isBitwiseMovable =
        NTSCFG_TYPE_CHECK_BITWISE_MOVABLE(ntsa::ArpHeader);

    NTSCFG_TEST_TRUE(isBitwiseMovable);

    const bool isBitwiseCopyable =
        NTSCFG_TYPE_CHECK_BITWISE_COPYABLE(ntsa::ArpHeader);
