...
BSLS_IDENT_RCSID(ntsa_udptimepoint_t_cpp, "$Id$ $CSID$")
...
// Provide tests for 'ntsa::UdpTimePoint'.
class UdpTimePointTest
{
  public:
...
NTSCFG_TEST_FUNCTION(ntsa::UdpTimePointTest::verifyTypeTraits)
{
    const bool isBitwiseInitializable =
        NTSCFG_TYPE_CHECK_BITWISE_INITIALIZABLE(ntsa::UdpTimePointInterval);

    NTSCFG_TEST_TRUE(isBitwiseInitializable);

    const bool isBitwiseMovable =
        NTSCFG_TYPE_CHECK_BITWISE_MOVABLE(ntsa::UdpTimePointInterval);

    NTSCFG_TEST_TRUE(isBitwiseMovable);

    const bool isBitwiseCopyable =
        NTSCFG_TYPE_CHECK_BITWISE_COPYABLE(ntsa::UdpTimePointInterval);
