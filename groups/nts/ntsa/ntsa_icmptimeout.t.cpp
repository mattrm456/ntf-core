...
BSLS_IDENT_RCSID(ntsa_icmptimeout_t_cpp, "$Id$ $CSID$")
...
// Provide tests for 'ntsa::IcmpTimeout'.
class IcmpTimeoutTest
{
  public:
...
NTSCFG_TEST_FUNCTION(ntsa::IcmpTimeoutTest::verifyTypeTraits)
{
    const bool isBitwiseInitializable =
        NTSCFG_TYPE_CHECK_BITWISE_INITIALIZABLE(ntsa::IcmpTimeout);

    NTSCFG_TEST_TRUE(isBitwiseInitializable);

    const bool isBitwiseMovable =
        NTSCFG_TYPE_CHECK_BITWISE_MOVABLE(ntsa::IcmpTimeout);

    NTSCFG_TEST_TRUE(isBitwiseMovable);

    const bool isBitwiseCopyable =
        NTSCFG_TYPE_CHECK_BITWISE_COPYABLE(ntsa::IcmpTimeout);
