...
BSLS_IDENT_RCSID(ntsa_ipv6header_t_cpp, "$Id$ $CSID$")
...
// Provide tests for 'ntsa::Ipv6Header'.
...
NTSCFG_TEST_FUNCTION(ntsa::Ipv6HeaderTest::verifyTypeTraits)
{
    const bool isBitwiseInitializable =
        NTSCFG_TYPE_CHECK_BITWISE_INITIALIZABLE(ntsa::Ipv6Header);

    NTSCFG_TEST_TRUE(isBitwiseInitializable);

    const bool isBitwiseMovable =
        NTSCFG_TYPE_CHECK_BITWISE_MOVABLE(ntsa::Ipv6Header);

    NTSCFG_TEST_TRUE(isBitwiseMovable);

    const bool isBitwiseCopyable =
        NTSCFG_TYPE_CHECK_BITWISE_COPYABLE(ntsa::Ipv6Header);
