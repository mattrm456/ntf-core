...
BSLS_IDENT_RCSID(ntsa_icmpredirect_t_cpp, "$Id$ $CSID$")
...
// Provide tests for 'ntsa::IcmpRedirect'.
class IcmpRedirectTest
{
  public:
...
NTSCFG_TEST_FUNCTION(ntsa::IcmpRedirectTest::verifyTypeTraits)
{
    const bool isBitwiseInitializable =
        NTSCFG_TYPE_CHECK_BITWISE_INITIALIZABLE(ntsa::IcmpRedirect);

    NTSCFG_TEST_TRUE(isBitwiseInitializable);

    const bool isBitwiseMovable =
        NTSCFG_TYPE_CHECK_BITWISE_MOVABLE(ntsa::IcmpRedirect);

    NTSCFG_TEST_TRUE(isBitwiseMovable);

    const bool isBitwiseCopyable =
        NTSCFG_TYPE_CHECK_BITWISE_COPYABLE(ntsa::IcmpRedirect);
