...
BSLS_IDENT_RCSID(ntsa_icmprouterrequest_t_cpp, "$Id$ $CSID$")
...
// Provide tests for 'ntsa::IcmpRouterRequest'.
class IcmpRouterRequestTest
{
  public:
...
NTSCFG_TEST_FUNCTION(ntsa::IcmpRouterRequestTest::verifyTypeTraits)
{
    const bool isBitwiseInitializable =
        NTSCFG_TYPE_CHECK_BITWISE_INITIALIZABLE(ntsa::IcmpRouterRequest);

    NTSCFG_TEST_TRUE(isBitwiseInitializable);

    const bool isBitwiseMovable =
        NTSCFG_TYPE_CHECK_BITWISE_MOVABLE(ntsa::IcmpRouterRequest);

    NTSCFG_TEST_TRUE(isBitwiseMovable);

    const bool isBitwiseCopyable =
        NTSCFG_TYPE_CHECK_BITWISE_COPYABLE(ntsa::IcmpRouterRequest);
