...
BSLS_IDENT_RCSID(ntsa_icmpchecksum_cpp, "$Id$ $CSID$")
...
IcmpChecksum::IcmpChecksum(const IcmpChecksum& original)
...
IcmpChecksum& IcmpChecksum::operator=(const IcmpChecksum& other)
{
    if (this != &other) {
...
void IcmpChecksum::reset()
{
    d_accumulator = 0;
}

void IcmpChecksum::add(const ntsa::Ipv4Address& sourceAddress,
                       ...
                       bsl::size_t              length)
{
    ...
    d_accumulator += static_cast<bsl::uint16_t>(IcmpChecksum::k_PROTOCOL_ICMP);
    d_accumulator += static_cast<bsl::uint16_t>(length);
}

void IcmpChecksum::add(const void* data, bsl::size_t size)
{
...
bsl::uint32_t IcmpChecksum::accumulator() const
{
    return d_accumulator;
}

bsl::uint16_t IcmpChecksum::value() const
{
