...
BSLS_IDENT_RCSID(ntsa_udpreassembly_t_cpp, "$Id$ $CSID$")
...
// Provide tests for 'ntsa::UdpReassembly'.
class UdpReassemblyTest
{
  public:
...
NTSCFG_TEST_FUNCTION(ntsa::UdpReassemblyTest::verifyTypeTraits)
{
    const bool isBitwiseInitializable =
        NTSCFG_TYPE_CHECK_BITWISE_INITIALIZABLE(ntsa::UdpReassembly);

    NTSCFG_TEST_TRUE(isBitwiseInitializable);

    const bool isBitwiseMovable =
        NTSCFG_TYPE_CHECK_BITWISE_MOVABLE(ntsa::UdpReassembly);

    NTSCFG_TEST_TRUE(isBitwiseMovable);

    const bool isBitwiseCopyable =
        NTSCFG_TYPE_CHECK_BITWISE_COPYABLE(ntsa::UdpReassembly);
